    ...
    if st.button("Check Backend Health"):
        try:
            ...
            if response.status_code == 200:
                ...
                st.info(f"Documents Indexed: {health.get('data_count', 0)}")
            else:
            ...
            with st.spinner("Uploading and indexing..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.toast("Ingestion started in the background!", icon="🚀")
                    else:
...
st.markdown("""
*Powered by **CrewAI** (Summarizer, Architect, Citation, Critic) & **LangGraph***
""")
        ...
        with status_placeholder.status("🚀 Crew is collaborating...", expanded=True) as status:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.write(f"📝 Summarizing context...")
                    st.write(f"⚖️ Verifying citations and auditing...")
                    
                    status.update(
                        label=f"✅ Complete ({latency})", 
                        state="complete", 
                        expanded=False
                    )
                ...
                st.error("The agents are taking a long time. Please check your logs.")
            except Exception as e:
